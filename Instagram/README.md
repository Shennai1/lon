...
当前版本为 **v1.3.8 紧凑遮罩加固版**。v1.3.7 已在 Mac 网页 App 冷启动后连续滚动 20 页且未出现空白，实测广告会变为 120px 小占位。v1.3.8 保持相同渲染方式，只增强入口与链接兼容性：

- GraphQL / Feed JSON 只检测广告标记并写入诊断信息，不删除、不替换任何响应正文；
- 不再拒绝 `api/v1/injected_story_units/`；
- 只处理包含 Meta 高置信广告跳转链接的最近一层 `article`；兼容绝对地址、同源相对地址、无尾斜杠和大小写变化；
...
- 首页响应规则兼容额外查询参数；即使旧式 `?__a=1` 返回 JSON，响应类型检查也只会观察而不会注入 CSS；
- JSON 诊断规则补充 `graphql_www` 与后续 `feed/*` 路径，但仍不修改响应正文；
...
- `instagram_web_ad_filter.v1.3.7.sgmodule`：已通过 Mac 20 页冷启动滚动测试的 120px 版本备份。
...
```text
https://raw.githubusercontent.com/Shennai1/lon/main/Instagram/instagram_web_ad_filter.sgmodule?v=1.3.8
```

v1.3.7 已验证备份：

```text
https://raw.githubusercontent.com/Shennai1/lon/main/Instagram/instagram_web_ad_filter.v1.3.7.sgmodule
```
...
`detected=N` 只表示脚本看到了高置信广告记录。v1.3.8 不会根据这个结果修改 JSON，因此它不能单独证明广告已在页面中被处理。

初始 HTML 命中时会出现：

```text
X-Surge-IG-Ad-Filter: ran; mode=html-compact; compact-height=120px; selector-rev=2; injected=1
```
...
- v1.3.5 的 80% 与 v1.3.6 的 50% 直接折叠都已在实机复现持续空白，因此 v1.3.7 起完全移除了 `display:none`。
- 120px 高度仍会改变 Instagram 信息流几何尺寸；当前 Mac 测试通过，但仍建议在 iOS 长时间验证。若再次空白，应回退为 100% 原高度遮罩。
- 加固只能降低规则失效概率，无法保证 Instagram 更换所有私有接口或广告 DOM 后仍然生效。
